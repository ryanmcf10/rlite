...
def menu(con, header, options, width, screen_width, screen_height):
    ...
    # create console to represent the window
...
# create a menu with each item of the inventory as an option
def inventory_menu(con, header, player, inventory_width, screen_width, screen_height):
    if len(player.inventory.items) == 0:
        ...
        for item in player.inventory.items:
            if player.equipment.main_hand == item or player.equipment.off_hand == item:
                options.append('{} [EQUIPPED]'.format(item.name))
            else:
...
def level_up_menu(con, header, player, menu_width, screen_width, screen_height):
    options = ['Consitution (+20 HP, from {})'.format(player.fighter.max_hp),
...
def main_menu(con, background_image, screen_width, screen_height):
    ...
    libtcod.console_print_ex(0, int(screen_width /2), int(screen_height - 2), libtcod.BKGND_NONE, libtcod.CENTER, 'R.M.')
