    ...
    while not libtcod.console_is_window_closed():
        ...
        if show_main_menu:
            ...
            if show_load_error_message:
                message_box(con, 'No save game data to load', 50, constants['screen_width'], constants['screen_height'])
...
def play_game(player, entities, game_map, message_log, game_state, con, panel, constants):
    ...
    while not libtcod.console_is_window_closed():
        ...
        if level_up and game_state == GameStates.LEVEL_UP:
            if level_up == 'hp':
        ...
        for player_turn_result in player_turn_results:
            ...
            if equip:
                ...
                for equip_result in equip_results:
                    ...
                    deequipped = equip_result.get('deequipped')

                    if deequipped and equipped:
                        message_log.add_message('You deequipped the {} and equipped the {}.'.format(deequipped.name, equipped.name))
                    elif equipped:
                        message_log.add_message(Message('You equipped the {}'.format(equipped.name)))
                    elif deequipped:
                        message_log.add_message(Message('You deequipped the {}.'.format(deequipped.name)))
            ...
            if xp:
                ...
                message_log.add_message(Message('You gain {} XP.'.format(str(xp))))

                if leveled_up:
                    message_log.add_message(Message('Your battle skills grow stronger! You reached level {}.'.format(str(player.level.current_level)), libtcod.yellow))
                    previous_game_state = game_state
